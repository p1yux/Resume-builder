        ...
        <div>
          <Card>
            <CardHeader className="px-4 py-3 flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-md font-medium">All Awards</CardTitle>
                      ...
                      <div key={`${award.resumeId}-${index}`} className="mb-4">
                        <div 
                          className={`p-3 rounded-md cursor-pointer hover:bg-slate-50 border-l-4 ${
                            selectedItem === award ? 'border-blue-500 bg-blue-50' : 'border-transparent'
                          }`}
                          onClick={() => handleSelectItem(award)}
                        >
                          <div className="font-medium">{award.title}</div>
                          <div className="text-xs text-muted-foreground mt-1">{award.date}</div>
                          <Badge variant="outline" className="mt-2 text-xs">
                            {award.resumeTitle}
                          </Badge>
                        </div>
                        {index < filteredAwards.length - 1 && <Separator className="my-2" />}
                      </div>
                    ))
                  )
                )}
